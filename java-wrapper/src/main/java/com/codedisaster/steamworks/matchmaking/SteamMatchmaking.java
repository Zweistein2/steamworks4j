...
public class SteamMatchmaking extends SteamInterface {
	...
	public enum LobbyComparison {
		...
		LobbyComparison(final int value) {
			this.value = value;
		}
	}
	...
	public enum ChatRoomEnterResponse {
		...
		ChatRoomEnterResponse(final int code) {
			this.code = code;
		}

		static ChatRoomEnterResponse byValue(final int code) {
			for (final ChatRoomEnterResponse value : values) {
				if (value.code == code) {
					return value;
				}
			}
	...
	public enum ChatMemberStateChange {
		...
		ChatMemberStateChange(final int bits) {
			this.bits = bits;
		}

		static boolean isSet(final ChatMemberStateChange value, final int bitMask) {
	...
	public enum ChatEntryType {
		...
		ChatEntryType(final int code) {
			this.code = code;
		}

		static ChatEntryType byValue(final int code) {
			for (final ChatEntryType value : values) {
				if (value.code == code) {
					return value;
				}
			}
	...
	public SteamMatchmaking(final SteamMatchmakingCallback callback) {
	...
	public boolean getFavoriteGame(final int game, final int[] appID, final int[] ip, final short[] connPort,
                                   final short[] queryPort, final int[] flags, final int[] lastPlayedOnServer) {
		return getFavoriteGame(pointer, game, appID, ip, connPort, queryPort, flags, lastPlayedOnServer);
	}

	public int addFavoriteGame(final int appID, final int ip, final short connPort, final short queryPort, final int flags, final int lastPlayedOnServer) {
		return addFavoriteGame(pointer, appID, ip, connPort, queryPort, flags, lastPlayedOnServer);
	}

	public boolean removeFavoriteGame(final int appID, final int ip, final short connPort, final short queryPort, final int flags) {
	...
	public void addRequestLobbyListStringFilter(final String keyToMatch,
                                                final String valueToMatch,
                                                final LobbyComparison comparisonType) {
		addRequestLobbyListStringFilter(pointer, keyToMatch, valueToMatch, comparisonType.value);
	}

	public void addRequestLobbyListNumericalFilter(final String keyToMatch,
                                                   final int valueToMatch,
                                                   final LobbyComparison comparisonType) {
		addRequestLobbyListNumericalFilter(pointer, keyToMatch, valueToMatch, comparisonType.value);
	}

	public void addRequestLobbyListNearValueFilter(final String keyToMatch, final int valueToBeCloseTo) {
		addRequestLobbyListNearValueFilter(pointer, keyToMatch, valueToBeCloseTo);
	}

	public void addRequestLobbyListFilterSlotsAvailable(final int slotsAvailable) {
		addRequestLobbyListFilterSlotsAvailable(pointer, slotsAvailable);
	}

	public void addRequestLobbyListDistanceFilter(final LobbyDistanceFilter lobbyDistanceFilter) {
		addRequestLobbyListDistanceFilter(pointer, lobbyDistanceFilter.ordinal());
	}

	public void addRequestLobbyListResultCountFilter(final int maxResults) {
		addRequestLobbyListResultCountFilter(pointer, maxResults);
	}

	public void addRequestLobbyListCompatibleMembersFilter(final SteamID steamIDLobby) {
		addRequestLobbyListCompatibleMembersFilter(pointer, steamIDLobby.handle);
	}

	public SteamID getLobbyByIndex(final int lobby) {
		return SteamID.createFromNativeHandle(getLobbyByIndex(pointer, lobby));
	}

	public SteamAPICall createLobby(final LobbyType lobbyType, final int maxMembers) {
		return new SteamAPICall(createLobby(pointer, callback, lobbyType.ordinal(), maxMembers));
	}

	public SteamAPICall joinLobby(final SteamID steamIDLobby) {
		return new SteamAPICall(joinLobby(pointer, callback, steamIDLobby.handle));
	}

	public void leaveLobby(final SteamID steamIDLobby) {
		leaveLobby(pointer, steamIDLobby.handle);
	}

	public boolean inviteUserToLobby(final SteamID steamIDLobby, final SteamID steamIDInvitee) {
		return inviteUserToLobby(pointer, steamIDLobby.handle, steamIDInvitee.handle);
	}

	public int getNumLobbyMembers(final SteamID steamIDLobby) {
		return getNumLobbyMembers(pointer, steamIDLobby.handle);
	}

	public SteamID getLobbyMemberByIndex(final SteamID steamIDLobby, final int memberIndex) {
		return SteamID.createFromNativeHandle(getLobbyMemberByIndex(pointer, steamIDLobby.handle, memberIndex));
	}

	public String getLobbyData(final SteamID steamIDLobby, final String key) {
		return getLobbyData(pointer, steamIDLobby.handle, key);
	}

	public boolean setLobbyData(final SteamID steamIDLobby, final String key, final String value) {
		return setLobbyData(pointer, steamIDLobby.handle, key, value);
	}

	public boolean setLobbyData(final SteamID steamIDLobby, final SteamMatchmakingKeyValuePair keyValuePair) {
		return setLobbyData(pointer, steamIDLobby.handle, keyValuePair.getKey(), keyValuePair.getValue());
	}

	public String getLobbyMemberData(final SteamID steamIDLobby, final SteamID steamIDUser, final String key) {
		return getLobbyMemberData(pointer, steamIDLobby.handle, steamIDUser.handle, key);
	}

	public void setLobbyMemberData(final SteamID steamIDLobby, final String key, final String value) {
		setLobbyMemberData(pointer, steamIDLobby.handle, key, value);
	}

	public void setLobbyMemberData(final SteamID steamIDLobby, final SteamMatchmakingKeyValuePair keyValuePair) {
		setLobbyMemberData(pointer, steamIDLobby.handle, keyValuePair.getKey(), keyValuePair.getValue());
	}

	public int getLobbyDataCount(final SteamID steamIDLobby) {
		return getLobbyDataCount(pointer, steamIDLobby.handle);
	}

	public boolean getLobbyDataByIndex(final SteamID steamIDLobby, final int lobbyDataIndex,
                                       final SteamMatchmakingKeyValuePair keyValuePair) {
		return getLobbyDataByIndex(pointer, steamIDLobby.handle, lobbyDataIndex, keyValuePair);
	}

	public boolean deleteLobbyData(final SteamID steamIDLobby, final String key) {
	 ...
	 */
	public boolean sendLobbyChatMsg(final SteamID steamIDLobby, final ByteBuffer data) throws SteamException {
	...
	public boolean sendLobbyChatMsg(final SteamID steamIDLobby, final String data) {
		return sendLobbyChatMsg(pointer, steamIDLobby.handle, data);
	}

	/**
	 * Read incoming chat entry into a {@link SteamMatchmaking.ChatEntry} structure,
	 * and a direct {@link ByteBuffer}.
	 */
	public int getLobbyChatEntry(final SteamID steamIDLobby, final int chatID,
                                 final ChatEntry chatEntry, final ByteBuffer dest) throws SteamException {
	...
	public boolean requestLobbyData(final SteamID steamIDLobby) {
		return requestLobbyData(pointer, steamIDLobby.handle);
	}

	public void setLobbyGameServer(final SteamID steamIDLobby, final int gameServerIP,
                                   final short gameServerPort, final SteamID steamIDGameServer) {
		setLobbyGameServer(pointer, steamIDLobby.handle, gameServerIP, gameServerPort, steamIDGameServer.handle);
	}

	public boolean getLobbyGameServer(final SteamID steamIDLobby, final int[] gameServerIP,
                                      final short[] gameServerPort, final SteamID steamIDGameServer) {
	...
	public boolean setLobbyMemberLimit(final SteamID steamIDLobby, final int maxMembers) {
		return setLobbyMemberLimit(pointer, steamIDLobby.handle, maxMembers);
	}

	public int getLobbyMemberLimit(final SteamID steamIDLobby) {
		return getLobbyMemberLimit(pointer, steamIDLobby.handle);
	}

	public boolean setLobbyType(final SteamID steamIDLobby, final LobbyType lobbyType) {
		return setLobbyType(pointer, steamIDLobby.handle, lobbyType.ordinal());
	}

	public boolean setLobbyJoinable(final SteamID steamIDLobby, final boolean joinable) {
		return setLobbyJoinable(pointer, steamIDLobby.handle, joinable);
	}

	public SteamID getLobbyOwner(final SteamID steamIDLobby) {
		return SteamID.createFromNativeHandle(getLobbyOwner(pointer, steamIDLobby.handle));
	}

	public boolean setLobbyOwner(final SteamID steamIDLobby, final SteamID steamIDNewOwner) {
		return setLobbyOwner(pointer, steamIDLobby.handle, steamIDNewOwner.handle);
	}

	public boolean setLinkedLobby(final SteamID steamIDLobby, final SteamID steamIDLobbyDependent) {
