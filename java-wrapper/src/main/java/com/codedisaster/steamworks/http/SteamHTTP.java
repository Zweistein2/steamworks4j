...
public class SteamHTTP extends SteamInterface {
	...
	public enum HTTPStatusCode {
		...
		public */
		static HTTPStatusCode byValue(final int statusCode) {
			int from = 0;
			...
			while (from <= to) {
				final int idx = (from + to) / 2;
				final HTTPStatusCode value = values[idx];
	...
	public SteamHTTP(final SteamHTTPCallback callback) {
		this(SteamAPI.getSteamHTTPPointer(),
			 createCallback(new SteamHTTPCallbackAdapter(callback)));
	}

	protected SteamHTTP(final long pointer, final long callback) {
		super(pointer, callback);
	}

	public SteamHTTPRequestHandle createHTTPRequest(final HTTPMethod requestMethod, final String absoluteURL) {
		return new SteamHTTPRequestHandle(createHTTPRequest(pointer, requestMethod.ordinal(), absoluteURL));
	}

	public boolean setHTTPRequestContextValue(final SteamHTTPRequestHandle request, final long contextValue) {
		return setHTTPRequestContextValue(pointer, request.handle, contextValue);
	}

	public boolean setHTTPRequestNetworkActivityTimeout(final SteamHTTPRequestHandle request, final int timeoutSeconds) {
		return setHTTPRequestNetworkActivityTimeout(pointer, request.handle, timeoutSeconds);
	}

	public boolean setHTTPRequestHeaderValue(final SteamHTTPRequestHandle request,
	...
	public boolean setHTTPRequestGetOrPostParameter(final SteamHTTPRequestHandle request,
	...
	public SteamAPICall sendHTTPRequest(final SteamHTTPRequestHandle request) {
		return new SteamAPICall(sendHTTPRequest(pointer, callback, request.handle));
	}

	public SteamAPICall sendHTTPRequestAndStreamResponse(final SteamHTTPRequestHandle request) {
		return new SteamAPICall(sendHTTPRequestAndStreamResponse(pointer, request.handle));
	}

	public int getHTTPResponseHeaderSize(final SteamHTTPRequestHandle request, final String headerName) {
		return getHTTPResponseHeaderSize(pointer, request.handle, headerName);
	}

	public boolean getHTTPResponseHeaderValue(final SteamHTTPRequestHandle request, final String headerName,
                                              final ByteBuffer value) throws SteamException {
	...
	public int getHTTPResponseBodySize(final SteamHTTPRequestHandle request) {
		return getHTTPResponseBodySize(pointer, request.handle);
	}

	public boolean getHTTPResponseBodyData(final SteamHTTPRequestHandle request, final ByteBuffer data) throws SteamException {
	...
	public boolean getHTTPStreamingResponseBodyData(final SteamHTTPRequestHandle request, final int bodyDataOffset,
                                                    final ByteBuffer data) throws SteamException {
	...
	public boolean releaseHTTPRequest(final SteamHTTPRequestHandle request) {
