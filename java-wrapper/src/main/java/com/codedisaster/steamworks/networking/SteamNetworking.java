...
public class SteamNetworking extends SteamInterface {
	...
	public enum P2PSessionError {
		...
		public static P2PSessionError byOrdinal(final int sessionError) {
	...
	public SteamNetworking(final SteamNetworkingCallback callback) {
		this(SteamAPI.getSteamNetworkingPointer(),
             createCallback(new SteamNetworkingCallbackAdapter(callback)));
	}

	protected SteamNetworking(final long pointer, final long callback) {
		super(pointer, callback);
	}

	public boolean sendP2PPacket(final SteamID steamIDRemote, final ByteBuffer data,
                                 final P2PSend sendType, final int channel) throws SteamException {
	...
	public int isP2PPacketAvailable(final int channel) {
	 ...
	 * sender's ID.
	 */
	public int readP2PPacket(final SteamID steamIDRemote, final ByteBuffer dest, final int channel) throws SteamException {
	...
	public boolean acceptP2PSessionWithUser(final SteamID steamIDRemote) {
		return acceptP2PSessionWithUser(pointer, steamIDRemote.handle);
	}

	public boolean closeP2PSessionWithUser(final SteamID steamIDRemote) {
		return closeP2PSessionWithUser(pointer, steamIDRemote.handle);
	}

	public boolean closeP2PChannelWithUser(final SteamID steamIDRemote, final int channel) {
		return closeP2PChannelWithUser(pointer, steamIDRemote.handle, channel);
	}

	public boolean getP2PSessionState(final SteamID steamIDRemote, final P2PSessionState connectionState) {
		return getP2PSessionState(pointer, steamIDRemote.handle, connectionState);
	}

	public boolean allowP2PPacketRelay(final boolean allow) {
